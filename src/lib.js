...
export function normalizeReverbState(value) {
  return String(value || "").trim().toLowerCase();
}

export function buildListingStateFields(rawState, qty) {
  const state = normalizeReverbState(rawState);
  const fields = {};
  if (state === "live") {
...
export function shouldDecrementOrder(rawStatus) {
  const status = normalizeReverbState(rawStatus);
...
export function isIgnoredOrder(rawStatus) {
  return CFG.ignoreStatuses.has(normalizeReverbState(rawStatus));
}
